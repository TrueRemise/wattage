...
label after_load:
    ...
    if persistent.lan_cd > time.time():
        $ lan_cd_start()
    else:
        $ persistent.lan_cd = 0
...
init python:
    ...
    def lan_cd_start():
        persistent.lan_cd = time.time() + 30

    def lan_save_scum_handling():
        if sol > persistent.lan_currency_last_save and (persistent.lan_cd or 0.0) > time.time():
